...
export async function POST(req: NextRequest) {
  ...
  const mailOptions = {
    ...
    to: process.env.GMAIL_USER, // o email que receberá as candidaturas
  ...
  try {
    ...
    return NextResponse.json({ message: 'Application submitted and email sent successfully!' });
  } catch (error) {
    console.error('Error sending email:', error);
    return NextResponse.json({ message: 'Application submitted but failed to send email.', error }, { status: 500 });
  }
}
